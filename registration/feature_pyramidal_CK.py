...
WORK_OUTPUT    = os.path.join(config.DATASPACE, "Feature_pyramidal_CK")
...
MIN_MATCHES_BY_LEVEL      = [20,   30,   40   ]  # pool fed into RANSAC — must be >> MIN_INLIERS
MIN_INLIERS_BY_LEVEL      = [6,    8,    10   ]  # survivors after RANSAC — conservative floor
...
MAX_TRANSLATION_PX = 400.0  # cumulative limit (rotation sanity gate)

# Per-level residual translation limits (full-res pixels).
# Coarse levels should produce small residuals — a huge tx/ty at L0 means
# RANSAC found a false consensus. Scale tightly with pyramid level.
MAX_RESIDUAL_TRANSLATION_BY_LEVEL = [100.0, 150.0, 300.0]
...
def detect_and_estimate_at_level(fixed_8bit, moving_8bit, level_scale, level_idx):
    ...
    if level_idx == 0:
        ...
        M_constrained = constrain_full_affine(M_partial, max_scale_deviation=0.02, max_shear=0.02)
...
def register_slice_pyramidal(fixed_np, moving_np, slice_id=None, diag_dir=None):
    ...
    for lvl_idx, scale in enumerate(PYRAMID_SCALES):
        ...
        if M_residual_fullres is not None:
            # Guard: reject the residual itself if its translation is physically absurd.
            # A large residual tx/ty means RANSAC locked onto a wrong consensus and
            # the coarse alignment will poison all finer levels downstream.
            res_tx     = abs(M_residual_fullres[0, 2])
            res_ty     = abs(M_residual_fullres[1, 2])
            max_res_t  = MAX_RESIDUAL_TRANSLATION_BY_LEVEL[lvl_idx]
            if res_tx > max_res_t or res_ty > max_res_t:
                logger.warning(
                    f"[{slice_id}] Level {lvl_idx}: residual translation too large "
                    f"(tx={M_residual_fullres[0,2]:.1f}px ty={M_residual_fullres[1,2]:.1f}px "
                    f"limit={max_res_t:.0f}px) — discarding residual, carrying previous transform."
                )
            else:
