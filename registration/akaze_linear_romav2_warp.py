...
WORK_OUTPUT       = os.path.join(config.DATASPACE, "Filter_AKAZE_RoMaV2_Linear_Warp")
...
def save_registration_plot(fixed_log, moving_log, moving_log_affine,
    ...
    If linear images are supplied (fixed_lin, moving_lin, moving_lin_affine),
    all overlay panels use them — showing exactly what RoMaV2 received.
    Falls back to log images if linear not supplied.
    """
    out_dir = os.path.join(OUTPUT_FOLDER, "interim_plots")
    os.makedirs(out_dir, exist_ok=True)
    ...
    # For remap we also need the actual array (not normed)
