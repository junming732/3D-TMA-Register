...
parser.add_argument('--core_name',          type=str, required=True,
                    help='Target core identifier')
parser.add_argument('--registration_mode',  type=str, default='ck_only',
                    choices=['ck_only', '3ch_fusion', 'color_lut'],
                    help='Which image is fed to AKAZE + tissue mask + B-spline')
args = parser.parse_args()

TARGET_CORE        = args.core_name
# BSPLINE_MODE: controls which image is fed to the B-spline level only.
# AKAZE (L0) and tissue mask always use CK regardless of this setting.
#   'ck_only'    — B-spline also uses CK (original behaviour end-to-end)
#   '3ch_fusion' — B-spline uses weighted DAPI + AF + CK fusion
#   'color_lut'  — B-spline uses 7-channel (AF excluded) color-LUT weighted-average -> gray
BSPLINE_MODE  = args.registration_mode
...
WORK_OUTPUT = os.path.join(config.DATASPACE, f"Filter_AKAZE_TissueMask_BSpline_multi_channel_{BSPLINE_MODE}")
...
# ── DAPI(0) + AF(7) + CK(6) weighted grayscale fusion ────────────────────────
DAPI_CHANNEL_IDX = 0
AF_CHANNEL_IDX   = 7
FUSION_WEIGHTS   = (1.0, 0.3, 0.6)   # (DAPI, AF, CK) — tune as needed

def _prepare_single(img_arr, lo=0.1, hi=99.5):
    """Log-normalize one channel to uint8."""
    img_float = img_arr.astype(np.float32)
    log_img   = np.log1p(img_float)
...
def prepare_3ch_fusion(img_arr_vol):
    """
    Weighted grayscale fusion of DAPI + AF + CK -> single uint8 detection image.
    Replaces prepare_ck() for AKAZE/tissue-mask/B-spline input.
    Returns (linear_uint8, fused_log_uint8) to match prepare_ck's signature.
    """
    dapi = _prepare_single(img_arr_vol[DAPI_CHANNEL_IDX].astype(np.float32)).astype(np.float32)
    af   = _prepare_single(img_arr_vol[AF_CHANNEL_IDX].astype(np.float32)).astype(np.float32)
    ck   = _prepare_single(img_arr_vol[CK_CHANNEL_IDX].astype(np.float32)).astype(np.float32)
    w_d, w_a, w_c = FUSION_WEIGHTS
    fused = np.clip((w_d * dapi + w_a * af + w_c * ck) / (w_d + w_a + w_c), 0, 255).astype(np.uint8)
    # linear image: use CK linear (same as current pipeline, for MSE calculation)
    ck_raw = img_arr_vol[CK_CHANNEL_IDX].astype(np.float32)
    p_lo, p_hi = np.percentile(ck_raw[::4, ::4], (1, 99.9))
    lin = cv2.normalize(np.clip(ck_raw, p_lo, p_hi), None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
    return lin, fused

# Channel -> RGB color LUT for 7-channel (AF excluded) weighted-average composite
COLOR_LUT = {
    0: (0,   128, 255),   # DAPI
    1: (51,  255,  51),   # CD31
    2: (255,  51,  51),   # GAP43
...
def prepare_color_lut_fusion(img_arr_vol):
    """
    7-channel (AF excluded) weighted-average color-LUT composite -> grayscale -> uint8.
    Returns (linear_uint8, gray_uint8) to match prepare_ck's signature.
    """
    c, h, w = img_arr_vol.shape
    ...
    composite = np.clip(acc / n * 255.0, 0, 255).astype(np.uint8)
...
def build_tissue_mask(ck_log: np.ndarray) -> np.ndarray:
    """
    High-fidelity binary tissue mask from the log-normalised detection image.
    
    Multi-stage pipeline (ported from visualize_registration_pair.py):
      1. Transient downsample to ~512px so morphological kernels cover the
         correct physical area regardless of full-res image size.
      2. Background removal via morphological opening + Otsu on the residual
         → "safe mask" that excludes TMA grid markings and canvas artefacts.
      3. Contrast stretch + Gaussian blur + Triangle threshold on the safe region.
         Triangle is more robust than Otsu when the background peak dominates.
      4. Intersect Triangle result with safe mask to prevent artefact leakage.
      5. Morphological close (fill holes) + open (remove specks).
      6. Upsample back to original resolution with INTER_NEAREST to keep binary.

    Returns a (H, W) uint8 array: 255 = tissue, 0 = background.
    """
    orig_h, orig_w = ck_log.shape

    # 1. Transient downsampling
    ...
    # 2. Background removal -> safe mask
    ...
    _, rough_mask  = cv2.threshold(foreground, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    ...
    # 3. Contrast stretch + Triangle threshold
    nonzero = img[img > 0]
    if len(nonzero) == 0:
    ...
    final_lr       = cv2.morphologyEx(closed,        cv2.MORPH_OPEN,  open_kern,   iterations=2)

    # 6. Upsample back to original resolution
    if scale < 1.0:
        final = cv2.resize(final_lr, (orig_w, orig_h), interpolation=cv2.INTER_NEAREST)
    else:
        final = final_lr
...
def register_slice(fixed_np, moving_np, slice_id=None):
    ...
    # L0 always uses CK — stable, high-contrast, sufficient keypoints for AKAZE
    ...
    # B-spline detection image — may differ from AKAZE image
    if BSPLINE_MODE == '3ch_fusion':
        _,  fixed_log_bspline  = prepare_3ch_fusion(fixed_np)
        _, moving_log_bspline  = prepare_3ch_fusion(moving_np)
    elif BSPLINE_MODE == 'color_lut':
        _,  fixed_log_bspline  = prepare_color_lut_fusion(fixed_np)
        _, moving_log_bspline  = prepare_color_lut_fusion(moving_np)
    else:  # 'ck_only'
        fixed_log_bspline  = fixed_log
        moving_log_bspline = moving_log
    ...
    if akaze_ok:
        ...
        if akaze_ok:
            ...
            if ncc_affine > ncc_raw:
                ...
                moving_log_bspline_affine = cv2.warpAffine(
                    moving_log_bspline, M_affine, (w, h),
                    flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT, borderValue=0,
                )
                aligned_np, ncc_value, elastic_ok, tile_stats, bspline_map_x, bspline_map_y = apply_bspline_l1(
                    fixed_log_bspline, moving_log_bspline_affine, moving_affine_vol, sid,
