...
AMOEBA_OUTPUT     = os.path.join(config.DATASPACE, "Amoeba_Registered_noGeometry")
INPUT_VOLUME_PATH = os.path.join(AMOEBA_OUTPUT, TARGET_CORE, f"{TARGET_CORE}_Amoeba_Aligned.ome.tif")
WORK_OUTPUT       = os.path.join(config.DATASPACE, "Feature_Registered_noGeometry")
...
CLAHE_TILE_GRID      = (8, 8)
MAX_CONDITION_NUMBER = 1e6    # Degenerate matrix guard — rejects numerically unstable affines
MAX_ROTATION_DEG     = 10.0  # Maximum allowed rotation in degrees — adjust to tissue biology
MAX_TRANSLATION_PX   = 400.0 # Maximum allowed translation in pixels — adjust to image dimensions
...
def register_slice_feature(fixed_np: np.ndarray, moving_np: np.ndarray):
    ...
    Validation chain:
      1. AKAZE convergence check
      2. Condition number guard (degenerate matrix)
      3. Forward displacement validation (rotation + translation bounds)
      4. Apply transform to all channels at full 16-bit precision
    """
    start = time.time()
    ...
    # Fallback 1: AKAZE convergence failure
    if M is None:
    ...
    # Fallback 2: Degenerate affine matrix guard
    cond = np.linalg.cond(M[:2, :2])
    if cond > MAX_CONDITION_NUMBER:
        logger.warning(f"[{detector}] Degenerate affine matrix rejected (cond={cond:.2e}). Reverting to Amoeba input.")
        aligned_np = moving_np.copy()
        mse        = float(np.mean((fixed_8bit.astype(np.float32) - moving_8bit.astype(np.float32)) ** 2))
        elapsed    = time.time() - start
        stats      = dict(detector="Amoeba_Input", n_good=n_good,
                          inlier_ratio=round(inlier_ratio, 3), rotation_deg=0.0, tx=0.0, ty=0.0, success=False)
        return aligned_np, mse, elapsed, stats, False

    # Fallback 3: Forward displacement validation using center-point mapping.
    # M maps moving→fixed (inverse direction), so M @ center gives the actual
    # physical displacement of the image center — stable regardless of matrix direction.
    cy, cx  = h / 2.0, w / 2.0
    center  = np.array([cx, cy, 1.0])
    mapped  = M @ center
    tx_fwd  = float(mapped[0] - cx)
    ty_fwd  = float(mapped[1] - cy)

    U, S, Vt = np.linalg.svd(M[:2, :2])
    R        = U @ Vt
    rot      = float(np.degrees(np.arctan2(R[1, 0], R[0, 0])))

    if abs(rot) > MAX_ROTATION_DEG or abs(tx_fwd) > MAX_TRANSLATION_PX or abs(ty_fwd) > MAX_TRANSLATION_PX:
        logger.warning(
            f"[{detector}] Transform exceeds geometric bounds "
            f"rot={rot:.2f} tx={tx_fwd:.1f} ty={ty_fwd:.1f}. Reverting to Amoeba input."
        )
        aligned_np = moving_np.copy()
        mse        = float(np.mean((fixed_8bit.astype(np.float32) - moving_8bit.astype(np.float32)) ** 2))
        elapsed    = time.time() - start
        stats      = dict(detector="Amoeba_Input", n_good=n_good,
                          ...
                          rotation_deg=round(rot, 3), tx=round(tx_fwd, 3), ty=round(ty_fwd, 3),
                          success=False)
        return aligned_np, mse, elapsed, stats, False

    # Apply validated transform to all channels at full 16-bit precision
    aligned_channels = []
...
def main():
    ...
    def process_pass(indices, direction):
        ...
        for i in indices:
            fixed_np  = aligned_vol[i + fixed_offset]   # chain from feature-corrected slice
            moving_np = vol_in[i]
            ...
            logger.info(
                ...
                f"Rot: {stats['rotation_deg']:.2f} | tx: {stats['tx']:.1f} ty: {stats['ty']:.1f} | "
