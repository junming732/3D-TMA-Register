"""
Feature registration — AKAZE affine + Landmark-Constrained B-spline elastic.
...
Pipeline per slice pair:
  ...
  L1  Landmark-constrained B-spline FFD (SimpleITK) → elastic residual correction.
      AKAZE inlier correspondences (mapped to the affine-prealigned frame) are
      passed to SimpleITK as PointSetMetric landmarks that anchor control points
      near matched features.  NCC intensity drives the interpolation everywhere
      else.  This replaces the pure intensity B-spline and removes the need to
      tune BSPLINE_GRID_NODES per core:
        - well-aligned cores  → few / small displacements at anchors, NCC term
                                 keeps the rest of the grid near identity.
        - heavily deformed cores → anchors pull the grid into large-but-valid
                                   deformations, NCC refines the in-between.
      Skipped if L0 failed (no valid pre-alignment to refine).

Landmark strategy:
  After the affine pre-warp the moving image is already close to the fixed.
  The residual displacements (fixed_pt − affine_moved_pt) are the landmarks
  passed to the B-spline as displacement targets.  Only a spatially-thinned
  subset (LANDMARK_MAX_PTS, selected by ANMS radius) is used so the point-set
  metric does not dominate the NCC term on well-aligned cores.
...
WORK_OUTPUT       = os.path.join(config.DATASPACE, "Filter_AKAZE_Landmark_BSpline_Zspace")
...
# --- DETECTOR ---
AKAZE_THRESHOLD = 0.0001   # lower than before — detect more features on weak CK signal
                            # (was 0.0003; RANSAC is the quality gate, not the detector)

# --- ANMS ---
ANMS_KEEP = 6000           # keep more keypoints before matching — was 2000
                            # More keypoints → more matches → more inliers → denser landmarks.
                            # ANMS still enforces spatial spread so they won't cluster.

# --- MATCHING ---
LOWE_RATIO  = 0.75         # tighter than before (was 0.80) — compensates for more raw keypoints
MIN_MATCHES = 20           # minimum matches fed into RANSAC (was 40 — relaxed for sparse cores)
MIN_INLIERS = 6            # minimum RANSAC survivors to accept the transform (was 8)
...
# --- LANDMARK-ONLY B-SPLINE ELASTIC REFINEMENT (L1) ---
#
# Strategy: pure feature-driven B-spline — no intensity optimizer.
#   AKAZE inlier correspondences (mapped to the affine-prealigned frame) are
#   passed directly to LandmarkBasedTransformInitializer which fits a B-spline
#   algebraically.  The warp is interpolated between anchors by the spline basis.
#
# BSPLINE_GRID_NODES — control points per axis.  More nodes = finer local
#   deformation between landmarks.  8 is a good default for ~6000px images.
# BSPLINE_MAX_DISPLACEMENT_PX — hard post-hoc cap on the displacement field
#   (safety net for degenerate landmark configurations).
# LANDMARK_MAX_PTS — max spatially-thinned inliers used as anchors.
#   400 is sufficient; beyond ~800 the spline becomes over-constrained.
BSPLINE_GRID_NODES          = 8
BSPLINE_MAX_DISPLACEMENT_PX = 80.0

# Landmark parameters
LANDMARK_MAX_PTS = 1000   # max AKAZE inliers used as displacement anchors (raised to match higher inlier yield)
...
def register_slice(fixed_np, moving_np, slice_id=None):
    ...
    # ── L1: Landmark-constrained B-spline (only when L0 gave a valid pre-alignment) ──
    ...
    if akaze_ok:
        ...
        # After the affine warp, moving inlier points map to new positions.
        # The residual error = fixed_pt − affine_warped_moving_pt defines the
        # per-landmark displacement the B-spline needs to correct.
        fixed_pts_lm  = None
        moving_pts_lm = None

        inlier_good = [m for m, keep in zip(good_matches, inlier_mask.ravel())
                       if keep] if len(inlier_mask) > 0 else []

        if len(inlier_good) >= 4:
            # Fixed points (destination in fixed image, x,y)
            fp = np.float32([kp1[m.queryIdx].pt for m in inlier_good])   # (N,2)
            # Moving points (source in original moving image, x,y)
            mp_orig = np.float32([kp2[m.trainIdx].pt for m in inlier_good])  # (N,2)

            # Apply affine to moving source points so they live in the
            # affine-prealigned frame (same space as fixed image now).
            ones      = np.ones((len(mp_orig), 1), dtype=np.float32)
            mp_h      = np.hstack([mp_orig, ones])              # (N,3)
            mp_warped = (M_affine @ mp_h.T).T                   # (N,2)  x,y

            # Residual: fixed_pt − warped_moving_pt
            # These are the displacements the B-spline must reproduce at each anchor.
            fixed_pts_lm  = fp          # where points should land
            moving_pts_lm = mp_warped   # where they currently are after affine

            logger.info(
                f"[{sid}] Passing {len(fp)} residual landmarks to B-spline "
...
def main():
    logger.info(f"Feature Registration (AKAZE + Landmark-constrained B-spline) — {TARGET_CORE}")
    ...
    df[cols].to_csv(
        os.path.join(OUTPUT_FOLDER, "registration_stats_AKAZE_Landmark_BSpline.csv"), index=False
    )
    ...
    generate_qc_montage(aligned_vol, OUTPUT_FOLDER, slice_ids=slice_ids,
                        channel_idx=CK_CHANNEL_IDX, channel_name="CK",
                        title_suffix="AKAZE+Landmark_BSpline")

    out_tiff = os.path.join(OUTPUT_FOLDER, f"{TARGET_CORE}_Landmark_Aligned.ome.tif")
