...
# NOTE: previously hardcoded os.environ['CUDA_VISIBLE_DEVICES'] = '' here, which
# forces torch.cuda.is_available() to False no matter what ROMAV2_DEVICE is set
# to below — removed so ROMAV2_DEVICE='cuda' actually takes effect. Set this env
# var externally (e.g. in the launch shell script) if you need to pin/restrict
# which GPU is visible.
...
ROMAV2_DEVICE            = 'cuda'  # falls back to 'cpu' automatically if unavailable (see get_romav2_model)
...
def prepare_3ch_fusion(img_arr_vol):
    """
    Returns (H, W, 3) uint8 RGB image for RoMaV2:
    DAPI->R, AF->G, CK->B, each channel log-normalised independently.
    """
    dapi = _prepare_single(img_arr_vol[DAPI_CHANNEL_IDX].astype(np.float32))
    af   = _prepare_single(img_arr_vol[AF_CHANNEL_IDX].astype(np.float32))
    ck   = _prepare_single(img_arr_vol[CK_CHANNEL_IDX].astype(np.float32))
...
def register_slice(fixed_np, moving_np, fixed_mask, moving_mask, slice_id=None):
    ...
    if ROMA_MODE == '3ch_fusion':
        fixed_roma_input  = prepare_3ch_fusion(fixed_np)
        moving_roma_input = prepare_3ch_fusion(moving_np)
    elif ROMA_MODE == 'color_lut':
    ...
    # NCC input — matches whatever channel(s) RoMaV2 actually sees this run
    # (per ROMA_MODE), NOT hardcoded to CK. 
    # Note: fixed_log/moving_log (CK-only) remain isolated for AKAZE detection 
    # (best structural features). However, both the NCC acceptance-gate and the 
    # diagnostic interim plots now dynamically reflect these ROMA_MODE inputs.
    fixed_ncc_img  = prepare_ncc_gray_log(fixed_roma_input)
    moving_ncc_img = prepare_ncc_gray_log(moving_roma_input)
    ...
    # affine_np already holds every channel warped by M_affine — reuse it
    # directly via prepare_roma_input rather than re-warping a separate
    # tracked image, so this can't drift out of sync with what actually got
    # warped.
    ncc_affine = measure_ncc(
        fixed_ncc_img.astype(np.float32),
        prepare_ncc_gray_log(prepare_roma_input(affine_np)).astype(np.float32),
        fixed_mask,
    )
    ...
    if tissue_frac >= MASK_MIN_FRAC:
        ...
        if map_x is not None:
            ...
            if np.count_nonzero(ck_out) / float(ck_out.size) < MIN_CK_NONZERO_FRAC:
                ...
                # NCC after warp — same channel(s) RoMaV2 actually warped,
                # reusing warp_candidate directly (all channels, already
                # warped) rather than re-deriving from a separately tracked
                # image.
                ncc_warp = measure_ncc(
                    fixed_ncc_img.astype(np.float32),
                    prepare_ncc_gray_log(prepare_roma_input(warp_candidate)).astype(np.float32),
                    fixed_mask,
                )
