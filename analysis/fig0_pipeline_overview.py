...
def fig0_conceptual_pipeline(out_path):
    ...
    W_sm, H_sm = 3.6, 1.8
    W_lg, H_lg = 5.2, 2.2
    W_prep = 3.9  # Accommodates slightly wider text in Pre-processing
    ...
    draw_block(ax, X_PREP, Y_FIXED, W_prep, H_sm, "Pre-processing", "Log/Lin Norm & Mask", 'white', C_FIXED)
    draw_block(ax, X_PREP, Y_MOVING, W_prep, H_sm, "Pre-processing", "Log/Lin Norm & Mask", 'white', C_MOVING)
    ...
    draw_arrow(ax, X_PREP + W_prep/2, Y_FIXED, X_AKAZE - W_lg/2 + 0.4, Y_ALGO + H_lg*0.5, rad=-0.1, color=C_FIXED)
    draw_arrow(ax, X_PREP + W_prep/2, Y_MOVING + 0.3, X_AKAZE - W_lg/2, Y_ALGO - 0.3, rad=0.15, color=C_MOVING)
    ...
    draw_arrow(ax, X_PREP + W_prep/2, Y_MOVING, X_AKAZE - W_lg/2, Y_MOVING, color=C_MOVING)
    ax.text((X_PREP + X_AKAZE - W_lg/2)/2 + 0.4, Y_MOVING - 1.1, "Fail L0 -> Warp Raw Image", ha='center', fontsize=16, color=C_FAIL, fontweight='bold')
    ...
    draw_arrow(ax, X_PREP + W_prep/2, Y_FIXED + 0.4, X_ROMA - W_lg/2 + 0.4, Y_ALGO + H_lg*0.5, rad=-0.1, color=C_FIXED)
    ...
    ax.text((X_AKAZE + W_lg/2 + X_ROMA - W_lg/2)/2, Y_MOVING - 1.1, "Fail L1 -> Retain Affine", ha='center', fontsize=16, color=C_FAIL, fontweight='bold')

    # Metric 2: Vector Filtering Gates 
    draw_metric_badge(ax, (X_AKAZE + X_ROMA)/2, Y_ALGO - 1.2, "Warp Filters:\nConf >= 0.5\nCap <= 200px\nZero Background", color=C_FILTER)

    # Metric 3: Global Output Gate 
    draw_metric_badge(ax, X_ROMA, Y_MOVING - 1.5, "Sanity Gate:\nNon-blank CK Output", color=C_FILTER)
