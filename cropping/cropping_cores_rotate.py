"""
TMA Core Extraction Pipeline
==========================================

This module implements an automated computer vision pipeline designed to identify, 
align, and extract individual tissue cores from high-resolution Tissue Microarray 
(TMA) whole-slide images.

The pipeline utilizes a multi-resolution approach, performing detection on 
downsampled layers for efficiency while extracting final data from high-resolution 
pyramidal layers via Zarr/Tifffile.

Key Architectural Steps:
------------------------
1. Image Loading: Reads OME-TIFF pyramids (levels 0 and low-res).
2. Preprocessing: 
   - Generates a "Safe Mask" to exclude background noise.
   - Applies linear contrast stretching and morphological operations.
   - Uses Triangle Thresholding for initial segmentation.
3. Candidate Detection:
   - Identifies contours with heuristic filtering (Area, Aspect Ratio).
   - Includes specific logic ("Monster Splitter") for separating fused cores 
     (specifically targeting Slice 3 artifacts).
4. Geometry Correction:
   - Calculates the rotation angle of the TMA grid based on row alignment.
   - Corrects grid tilt (clamped to a max limit) to ensure upright cores.
5. Grid Logic & Sorting:
   - Sorts candidates by size to select the most relevant cores.
   - Groups cores into logical rows (Y-axis) and columns (X-axis) to assign 
     sequential IDs matching the physical grid layout.
6. High-Res Extraction:
   - Maps rotational coordinates back to Level 0 dimensions.
   - Extracts raw pixel data (preserving 16-bit depth).
   - Writes individual multi-channel OME-TIFF files with metadata.
   - Generates JPEG thumbnails for rapid visual QC.

Global Constants (Internal):
    - PIXEL_SIZE_UM: 0.4961 (Physical pixel size for metadata)
    - CHANNEL_NAMES: List of specific biomarkers (DAPI, CD31, etc.)
"""
...
def crop_and_group_by_core():
    # Output to DATASPACE
    output_base = os.path.join(config.DATASPACE, "TMA_Cores_Grouped_Rotate")
            ...
            with tifffile.TiffFile(file_path) as tif:
                ...
                for idx, core in enumerate(ordered_cores):
                    ...
                    if detected_angle != 0.0:
                        # Cast to float32 for safe cv2 transformation
                        crop = np.ascontiguousarray(crop, dtype=np.float32) 
                        c, h, w = crop.shape
                        center = (w // 2, h // 2)
                        
                        # Get rotation matrix to straighten the core
                        M_rot = cv2.getRotationMatrix2D(center, detected_angle, 1.0)
                        
                        # Process each channel independently to bypass OpenCV's 4-channel limit
                        rotated_channels = []
                        for i in range(c):
                            single_channel = crop[i, :, :]
                            rotated_single = cv2.warpAffine(
                                single_channel, 
                                M_rot, 
                                (w, h), 
                            ...
                            rotated_channels.append(rotated_single)
